...
class Measurement:
    ...
    def __init__(self, num_frame, z, sensor):
        ...
        self.sensor: Sensor = sensor # sensor that generated this measurement
        ...
        if sensor.name == 'lidar':
            ...
            # z = [cente_x, center_y, width, length] of the bounding box
            self.z[0] = z[0]
            self.z[1] = z[1]
            # self.R = np.matrix([[sigma_cam_i**2, 0], [0, sigma_cam_j**2]])
            self.R = np.matrix([[(z[2]*0/4+sigma_cam_i)**2, 0], [0, (z[3]*0/4+sigma_cam_j)**2]])
            ############
            # END student code
            ############
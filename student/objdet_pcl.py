...
def load_range_image(frame, lidar_name):
    ...
    lidar = next(filter(lambda obj: obj.name == lidar_name, frame.lasers), None)
    ri = []
    if lidar: # use first response
...
def show_range_image(frame, lidar_name):
    ...
    thresholds = np.percentile(ri_intensity, [1,99])
    ri_intensity[ri_intensity<thresholds[0]] = thresholds[0]
    ri_intensity[ri_intensity>thresholds[1]] = thresholds[1]
    scale = 255/(thresholds[1]-thresholds[0])
    ri_intensity = (ri_intensity-thresholds[0])*scale

    # step 6 : stack the range and intensity image vertically using np.vstack and convert the result to an unsigned 8-bit integer
    img_range = ri_range.astype(np.uint8)
    img_intensity = ri_intensity.astype(np.uint8)
    img_range_intensity = cv2.vconcat([img_range, img_intensity])
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    min_x = configs.lim_x[0]
    min_y = np.min([configs.lim_y[0],-configs.lim_y[1]])
    max_y = np.max([-configs.lim_y[0],configs.lim_y[1]])
    min_z = configs.lim_z[0]

    delta_x = (configs.lim_x[1] - min_x)/configs.bev_height
    delta_y = (max_y - min_y)/configs.bev_width

    ## step 2 : create a copy of the lidar pcl and transform all metrix x-coordinates into bev-image coordinates
    # step 3 : perform the same operation as in step 2 for the y-coordinates but make sure that no negative bev-coordinates occur
    lidar_pcl_cpy = np.copy(lidar_pcl)
    lidar_pcl_cpy[:,0] = np.int_(np.floor((lidar_pcl[:, 0]-min_x) / delta_x))
    lidar_pcl_cpy[:,1] = np.int_(np.floor((lidar_pcl[:, 1]-min_y) / delta_y))
    lidar_pcl_cpy[:,2] = lidar_pcl[:, 2]-min_z
    ...
    lidar_pcl_top[:,3] = lidar_pcl_cpy[indices,3]
